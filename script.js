...
// Configuration optimisée pour les requêtes serveur
const SERVER_CONFIG = {
    maxRetries: 3,
    baseTimeout: 30000, // 30 secondes de base
    maxTimeout: 300000, // 5 minutes maximum
    retryDelay: 1000, // 1 seconde de délai initial
    maxRetryDelay: 10000, // 10 secondes maximum
    chunkSize: 1024 * 1024 * 5, // 5MB chunks pour gros fichiers
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initAnimations();
    setCurrentDate();
    checkServerStatus(); // Vérifier le statut du serveur au démarrage
...
async function checkServerStatus() {
    try {
        ...
        console.warn('Serveur inaccessible:', error.message);
    }
}

// Chargement des ondelettes optimisé
async function loadWavelets() {
    try {
        const response = await optimizedFetch(`${SERVER_URL}/api/wavelets`);
        ...
        // Fallback avec ondelettes par défaut
        const defaultWavelets = ['haar', 'db4', 'db8', 'bior2.2', 'coif2'];
        const selects = document.querySelectorAll('[id$="dwt-wavelet"]');
        selects.forEach(select => {
            select.innerHTML = '';
            defaultWavelets.forEach(wavelet => {
                const option = document.createElement('option');
                option.value = wavelet;
                option.textContent = wavelet;
                if (wavelet === 'haar') option.selected = true;
...
// Vérification de la configuration Storj
...
// Calcul du timeout dynamique basé sur la taille du fichier
function calculateTimeout(fileSize) {
    const baseTimeout = SERVER_CONFIG.baseTimeout;
    const sizeMultiplier = Math.max(1, fileSize / (1024 * 1024)); // MB
    const calculatedTimeout = baseTimeout + (sizeMultiplier * 10000); // +10s par MB
    return Math.min(calculatedTimeout, SERVER_CONFIG.maxTimeout);
}

// Requête optimisée avec retry et timeout dynamique
async function optimizedFetch(url, options = {}, fileSize = 0) {
    const requestId = Date.now() + Math.random();
    let lastError = null;
    
    for (let attempt = 0; attempt <= SERVER_CONFIG.maxRetries; attempt++) {
        try {
            // Calculer le timeout dynamique
            const timeout = calculateTimeout(fileSize);
            const controller = new AbortController();
            
            // Stocker la requête active pour pouvoir l'annuler
            ...
            // Ne pas retry si l'utilisateur a annulé
            ...
            const delay = Math.min(
                SERVER_CONFIG.retryDelay * Math.pow(2, attempt),
                SERVER_CONFIG.maxRetryDelay
            );
            
            console.warn(`Tentative ${attempt + 1} échouée, retry dans ${delay}ms:`, error.message);
            await new Promise(resolve => setTimeout(resolve, delay));
            
            // Revérifier le statut du serveur
            await checkServerStatus();
        }
    }
    
    throw new Error(`Échec après ${SERVER_CONFIG.maxRetries + 1} tentatives: ${lastError?.message || 'Erreur inconnue'}`);
}

// Insertion du watermark dans l'audio optimisée
async function embedWatermarkInAudio(watermarkText, audioFile) {
    ...
    try {
        const response = await optimizedFetch(`${SERVER_URL}/api/embed`, {
            method: 'POST',
            body: formData
        }, audioFile.size);
        ...
        if (error.message.includes('timeout') || error.message.includes('fetch')) {
            throw new Error('Timeout du serveur. Le fichier est peut-être trop volumineux ou le serveur est surchargé. Veuillez réessayer.');
        }
        throw error;
    }
}

// Extraction du watermark depuis l'audio optimisée
async function extractWatermarkFromAudio(audioFile) {
    ...
    try {
        const response = await optimizedFetch(`${SERVER_URL}/api/extract`, {
            method: 'POST',
            body: formData
        }, audioFile.size);
...
// Fonction pour annuler toutes les requêtes actives
function cancelActiveRequests() {
    ...
    showToast('Traitement annulé');
}

// Processus d'embedding optimisé
async function processEmbedding() {
    ...
    if (selectedAudioFile.size > 100 * 1024 * 1024) { // 100MB
        showStatus('Fichier trop volumineux (max 100MB). Veuillez compresser votre fichier.', 'warning', 'embed-status');
        return;
    }
    ...
    if (serverStatus === 'offline') {
        showStatus('Serveur inaccessible. Vérifiez votre connexion internet et réessayez.', 'danger', 'embed-status');
        return;
    }
    ...
    // Ajouter bouton d'annulation
    const originalButtonText = button.innerHTML;
    button.disabled = true;
    button.innerHTML = '<i class="fa-solid fa-spinner fa-spin mr-2"></i>Traitement en cours... <button onclick="cancelActiveRequests()" class="ml-2 text-red-500 hover:text-red-700">Annuler</button>';
    ...
    try {
        ...
        const estimatedTime = Math.ceil(selectedAudioFile.size / (1024 * 1024) * 2); // 2s par MB
        showStatus(`Traitement estimé: ${estimatedTime} secondes`, 'info', 'embed-status');
        ...
        if (error.message.includes('timeout')) {
            errorMessage = 'Timeout: Le traitement prend plus de temps que prévu. Veuillez réessayer avec un fichier plus petit.';
        } else if (error.message.includes('network')) {
            errorMessage = 'Erreur réseau: Vérifiez votre connexion internet.';
        } else if (error.message.includes('cancelled')) {
            errorMessage = 'Traitement annulé par l\'utilisateur.';
        }
        
        showStatus(`Erreur: ${errorMessage}`, 'danger', 'embed-status');
    } finally {
...
// Processus d'extraction optimisé
async function processExtraction() {
    ...
    if (audioFile.size > 100 * 1024 * 1024) { // 100MB
        showStatus('Fichier trop volumineux (max 100MB). Veuillez compresser votre fichier.', 'warning', 'extract-status');
        return;
    }
    ...
    await checkServerStatus();
    if (serverStatus === 'offline') {
        showStatus('Serveur inaccessible. Vérifiez votre connexion internet et réessayez.', 'danger', 'extract-status');
        return;
    }
    ...
    const originalButtonText = button.innerHTML;
    button.disabled = true;
    button.innerHTML = '<i class="fa-solid fa-spinner fa-spin mr-2"></i>Extraction en cours... <button onclick="cancelActiveRequests()" class="ml-2 text-red-500 hover:text-red-700">Annuler</button>';
    progress.classList.remove('hidden');
    result.classList.add('hidden');
    ...
    try {
        // Afficher l'estimation du temps
        const estimatedTime = Math.ceil(audioFile.size / (1024 * 1024) * 1.5); // 1.5s par MB
        showStatus(`Extraction estimée: ${estimatedTime} secondes`, 'info', 'extract-status');
        ...
        let errorMessage = error.message;
        if (error.message.includes('timeout')) {
            errorMessage = 'Timeout: L\'extraction prend plus de temps que prévu. Veuillez réessayer.';
        } else if (error.message.includes('network')) {
            errorMessage = 'Erreur réseau: Vérifiez votre connexion internet.';
        } else if (error.message.includes('cancelled')) {
            errorMessage = 'Extraction annulée par l\'utilisateur.';
        }
        
        showStatus(`Erreur: ${errorMessage}`, 'danger', 'extract-status');
    } finally {
        button.disabled = false;
        button.innerHTML = originalButtonText;
    }
